...
- Entain + Punterstech matcher-market hygiene (Phase A PR4):
  - sport-aware matcher market allow-list
  - explicit deny-list for futures/outrights/partials/prop-like markets
  - expected market shape enforcement (`home/draw/away` for soccer; `home/away` for nba/nhl/boxing/nbl)
  - non-deterministic `selection_key` rows are dropped before persistence path
- PR7 EPL fetch/filter hardening for Punterstech:
  - competition filter matching now normalizes punctuation/hyphen variants
  - default soccer `next-to-go` result limit increased to avoid EPL truncation on high-volume brands
...
- PR4 maintenance cleanup retired stale `is_current=true` matcher rows that violated
  the new Entain/Punterstech market hygiene policy (disallowed markets, invalid
  selection keys, or invalid market shape).
...
- PR4 hygiene evidence includes before/after market mismatch deltas and DB query outputs
  under `docs/evidence/phase-a-hardening/2026-02-11/`
- PR6 adds competition-aware validation calibration for final scoring:
  - event-count ceiling tuning for high-volume windows
  - competition-specific event-coverage thresholds (`epl/nba/nhl/nbl/boxing`)
  - competition-specific anomaly scoring thresholds (warn + critical)
- PR6.1/PR7 explicit bookmaker x competition coverage scope policy:
  - out-of-scope => `SKIP` (neutral, non-gating)
  - in-scope + zero-event => `FAIL` (blocking)
  - explicit coverage policy mappings:
    - `nbl` out-of-scope: `betblitz`, `betreal`, `starsports`, `truebet`, `wizbet`
    - `boxing` in-scope: `ladbrokes`, `neds`, `betfair` (others out-of-scope)
  - PR-R4A policy drift remediation (2026-02-21) updated `nbl` scope to classify
    `betreal` as out-of-scope after repeated live canary in-scope 0% coverage FAILs.
- local canary evidence is generated by:
